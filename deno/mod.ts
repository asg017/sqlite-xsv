...
let path: string;
try {
  const customPath = Deno.env.get("DENO_SQLITE_XSV_PATH");
  if (customPath) path = customPath;
  else {
    path = await download({
      url: {
        darwin: {
          aarch64: `${BASE}/deno-darwin-aarch64.xsv0.dylib`,
          x86_64: `${BASE}/deno-darwin-x86_64.xsv0.dylib`,
        },
        windows: {
          x86_64: `${BASE}/deno-windows-x86_64.xsv0.dll`,
        },
        linux: {
          x86_64: `${BASE}/deno-linux-x86_64.xsv0.so`,
        },
      },
      suffixes: {
        darwin: "",
  ...
  const error = new Error("Failed to load sqlite-xsv extension");
  error.cause = e;
...
/**
 * Returns the full path to the compiled sqlite-xsv extension.
 * Caution: this will not be named "xsv0.dylib|so|dll", since plug will
...
/**
 * Entrypoint name for the sqlite-xsv extension.
 */
export const entrypoint = "sqlite3_xsv_init";
...
/**
 * Loads the sqlite-xsv extension on the given sqlite3 database.
 */
